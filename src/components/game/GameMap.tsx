...
// Monólogos por personalidad
const SPEECH: Record<string, string[]> = {
  quejica:    ['¡Este arroz está seco!', 'Llevo esperando media hora', 'En mis tiempos esto no pasaba', '¿Pero es que nadie trabaja aquí?', 'La calefacción, otra vez mal'],
  cotilla:    ['¿Viste lo que hizo Don Paco?', 'Entre tú y yo...', 'Eso que te cuento, pero callao', 'Yo ya lo sabía...', 'Me ha llegado que...'],
  mandón:     ['Aquí mando yo', 'JR, que no tenemos todo el día', 'En el banco teníamos otro nivel', 'Eso lo haría yo diferente'],
...
// Diálogos entre pares de personalidades en la misma sala
const PAIR_DIALOGS: Record<string, { a: string; b: string }[]> = {
  'quejica-cotilla':  [{ a: '¿Has visto lo que sirven hoy?', b: 'Ya me lo contaron... no te imaginas' }],
  'mandón-quejica':   [{ a: 'Aquí mando yo, ¿queda claro?', b: '¡A mí no me mandes!' }],
  'coqueta-sordo':    [{ a: '¿Me ves bien así, Alfredo?', b: '¡Muy bien, gracias!' }],
  'devota-coqueta':   [{ a: 'Deberías rezar más', b: 'Ya tengo a quien rezarle, cielo' }],
  'mandón-cotilla':   [{ a: 'Reunión a las tres', b: '¡Ya lo sabe toda la planta!' }],
  'quejica-exigente': [{ a: 'La comida de hoy...', b: 'Eso no es cocina, eso es tragedia' }],
  'misterioso-normal':[{ a: '...', b: '¿Estás bien, Pepe?' }],
  'sordo-devota':     [{ a: '¿Hay misa?', b: '¡Claro que sí!' }],
}
...
const SPEECH_EVENT: Record<string, string[]> = {
  ...
  family_visit:  ['¡Que vienen mis hijos!', 'Espero que traigan turrones', 'Por fin una visita'],
}
...
const EVENT_OVERLAY: Record<string, { emoji: string; color: string }> = {
  ...
  family_visit:  { emoji: '👨‍👩‍👧',  color: 'rgba(190,24,93,0.75)'   },
}
...
export default function GameMap({ residence, residents, events, rooms, onResolve, onGoToBuild, onRepairRoom, onOpenMiniGame, onFamilyVisit }: Props) {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      // Find two residents in the same zone with a known pair dialog
      for (const r1 of residents) {
        for (const r2 of residents) {
          if (r1.id === r2.id) continue
          if (r1.current_room_type !== r2.current_room_type) continue
          const key = [r1.personality, r2.personality].sort().join('-')
          const dialogs = PAIR_DIALOGS[key]
          if (!dialogs) continue
          if (Math.random() > 0.4) continue
          const d = dialogs[Math.floor(Math.random() * dialogs.length)]
          // Assign A/B based on which personality comes first
          const aIsR1 = r1.personality === key.split('-')[0]
          setPairBubble({ aId: aIsR1 ? r1.id : r2.id, bId: aIsR1 ? r2.id : r1.id, aText: d.a, bText: d.b })
          setTimeout(() => setPairBubble(null), 4500)
          return
        }
      }
  ...
  // Get pixel position for a resident
  function getResidentPx(residentId: string): { x: number; y: number } | null {
    ...
    return { x: px.x + px.w * xFrac - 16, y: px.y + 20 }
  }
  ...
  async function handleResolve(event: GameEvent) {
    ...
    haptic(event.urgency === 'critical' ? [80, 40, 80, 40, 120] : [50, 30, 80])

    // Family visit → special dialog
    if (event.type === 'family_visit' && onFamilyVisit) {
      onFamilyVisit(event, resident)
      return
    }

    // Minigame event types
    ...
    <div className="flex flex-col gap-3">
      ...
      {brokenRoom && onRepairRoom && (
        <div className="card border-red-700/60 bg-red-950/20 flex items-center gap-3">
          ...
          <div className="flex-1">
            ...
            <p className="text-red-700 text-xs">Entretenimiento se degrada el doble hasta repararla</p>
          </div>
      ...
      {residents.some(r => r.activity === 'hospitalizado — esperando traslado') && (
        <div className="card border-red-600/70 bg-red-950/30 animate-pulse">
          ...
          <p className="text-red-700 text-xs">Una urgencia no atendida a tiempo ha requerido intervención médica. −500€ y −10 reputación.</p>
        </div>
      )}
      ...
      <div ref={containerRef} className="relative w-full rounded-2xl overflow-hidden border border-amber-900/40" style={{ height: `${totalH}px`, background: '#08080f' }}>
        ...
        {zones.filter(z => z.type !== 'empty').map(zone => {
          ...
          return (
            <div key={zone.id} className="absolute transition-all duration-300" style={{
              ...
              background: zone.cfg.bg, border: `1px solid ${isBroken ? '#dc2626' : zone.cfg.border}`,
              borderRadius: 10,
              boxShadow: hasCritical ? `0 0 14px 3px rgba(239,68,68,0.35)` : isBroken ? `0 0 10px 2px rgba(220,38,38,0.2)` : `inset 0 0 24px rgba(0,0,0,0.5)`,
            }}>
              <div className="flex items-center gap-1 px-2 pt-1.5">
                ...
                {roomData && roomData.level > 1 && (
                  <span className="text-[9px] text-amber-700 ml-auto">Nv.{roomData.level}</span>
                )}
                {zoneEvs.length > 0 && (
                  <span className={`ml-auto text-[9px] font-bold px-1 py-0.5 rounded-full ${hasCritical ? 'bg-red-600 text-white animate-pulse' : 'bg-orange-700 text-orange-200'}`}>
                    {zoneEvs.length}
                  </span>
                )}
              </div>
                    ...
                    return (
                      <button key={ev.id}
                        ...
                        <span className="text-white text-[9px] font-semibold truncate flex-1">
                          {ev.residents?.name} · {resolving === ev.id ? '...' : '👆'}
                        </span>
        ...
        {residents.map(resident => {
          ...
          const pairA = pairBubble?.aId === resident.id ? pairBubble.aText : null
          const pairB = pairBubble?.bId === resident.id ? pairBubble.bText : null
          const activeBubble = bubble || pairA || pairB
          ...
          return (
            <div key={resident.id} className="absolute pointer-events-none" style={{
              ...
              <div className={`text-2xl text-center leading-none ${isHospitalized ? 'opacity-40' : isWalking ? 'animate-walk' : 'animate-idle'}`}
                style={{ filter: `drop-shadow(0 0 4px ${happColor}40)` }}>
        ...
        {jrVisible && jrTarget && (
          <div className="absolute pointer-events-none z-20" style={{
            ...
            <div className="text-2xl text-center animate-walk">👨‍⚕️</div>
...
function StatsBar({ residence, events, residents, dayLabel }: { residence: Residence; events: GameEvent[]; residents: Resident[]; dayLabel: string }) {
  ...
  return (
    <div className="flex flex-col gap-1.5">
      ...
      <p className="text-amber-800 text-[10px] text-center">{dayLabel}</p>
    </div>
  )
}
